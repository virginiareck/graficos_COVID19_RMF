# -*- coding: utf-8 -*-
"""9Cópia de Cópia de Cópia de Cópia de Cópia de Cópia de Cópia de Cópia de Cópia de OrganizaDados-13-06-22.ipynb
...
fig = px.line(cidades, x="date", y="new_confirmed", color="city")
fig.show()
...
cidades_2020_agrupado_novos_ano_sum = cidades_2020.groupby(['mes','ano',])['new_confirmed','new_deaths'].sum().reset_index()
...
fig_2020_mortes_ano = px.bar(cidades_2020_agrupado_novos_ano,x='ano',y='new_deaths',color='city',barmode='group',title="Número de novos óbitos de COVID-19 na RMF no ano de 2020 por cidade.",text='new_deaths')
...
fig = px.bar(cidades_2021_agrupado_novos_mes,x='mês',y='new_confirmed',color='city',barmode='group',title="Número de novos casos confirmados de COVID-19 na RMF no ano de 2021.",text='new_confirmed')
fig.update_traces(textposition='outside')
fig.update_xaxes(type='category')
fig.show()